(function (global) {
    ...
    const root = global.CTTF = global.CTTF || {};
    root.modules = root.modules || [];

    root.modules.push(function registerSettingsModule(ctx) {
        if (!ctx) {
            console.warn('[Chat] Template Text Folders] Module context missing for settings module.');
            return;
        }
        ...
        const applyDomainStyles = (...args) => ctx.applyDomainStyles ? ctx.applyDomainStyles(...args) : undefined;
        const updateButtonBarLayout = (...args) => ctx.updateButtonBarLayout ? ctx.updateButtonBarLayout(...args) : undefined;
        const applyBarBottomSpacing = (...args) => ctx.applyBarBottomSpacing ? ctx.applyBarBottomSpacing(...args) : undefined;
        const updateStylesOnThemeChange = (...args) => ctx.updateStylesOnThemeChange ? ctx.updateStylesOnThemeChange(...args) : undefined;

        let { buttonConfig } = ctx;
        const defaultConfig = ctx.defaultConfig;
        ...
        ctx.buttonConfig = buttonConfig;
        ctx.createButtonContainer = typeof createButtonContainer === 'function' ? createButtonContainer : ctx.createButtonContainer;
        ctx.updateButtonContainer = typeof updateButtonContainer === 'function' ? updateButtonContainer : ctx.updateButtonContainer;
        ctx.showAutomationSettingsDialog = typeof showAutomationSettingsDialog === 'function' ? showAutomationSettingsDialog : ctx.showAutomationSettingsDialog;
        ctx.showStyleSettingsDialog = typeof showStyleSettingsDialog === 'function' ? showStyleSettingsDialog : ctx.showStyleSettingsDialog;
    });
